...
test('aes-256-gcm, null nonce', async (t) => {
    const key = data.slice(0, 32);
    const res = await encrypt(data, key, null);
    const out = await decrypt(res, key, null);
    t.true(Buffer.from(out).compare(data) === 0);
});

test('aes-256-gcm, non-null nonce', async (t) => {
    const key = data.slice(0, 32);
    const nonce = data.slice(32, 32 + 12);
    const res = await encrypt(data, key, nonce);
    const out = await decrypt(res, key, nonce);
    t.true(Buffer.from(out).compare(data) === 0);
});

test('aes-256-ctr, null nonce', async (t) => {
    const key = data.slice(0, 32);
    const res = await encrypt(data, key, null, false);
    const out = await decrypt(res, key, null, false);
    t.true(Buffer.from(out).compare(data) === 0);
});

test('aes-256-ctr, non-null nonce', async (t) => {
    const key = data.slice(0, 32);
    const nonce = data.slice(32, 32 + 16);
    const res = await encrypt(data, key, nonce, false);
    const out = await decrypt(res, key, nonce, false);
    t.true(Buffer.from(out).compare(data) === 0);
});
